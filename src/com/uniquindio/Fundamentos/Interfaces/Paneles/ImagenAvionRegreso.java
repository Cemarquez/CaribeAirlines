...
import com.uniquindio.Fundamentos.Interfaces.Regreso.VentanaPagoRegreso;
...
public class ImagenAvionRegreso extends JPanel implements ActionListener {
	...
	private ImagenAvionRegreso p = this;
	 ...
	 */
	public ImagenAvionRegreso(Vuelo vueloSeleccionado, int pasajerosTotal, Cliente miClienteLogeado,
			VentanaPrincipal miVentanaPrincipal, ArrayList<Pasajero> pasajeros) {
		...
		this.fechaaRegreso = vueloSeleccionado.getFecha();
			...
			if (arg0.getSource() == BotonesSillasEje[i]) {
				if (contador < pasajerosTotal) {
					...
					if (miSilla.sillaAsignada() == true) {
						...
						BotonesSillasEje[i].setIcon(
								new ImageIcon(VentanaSillas.class.getResource("/imagenes/silla-ejec-asig.png")));
						VentanaPagoRegreso miVP = new VentanaPagoRegreso(pasajeros, vueloSeleccionado, sillasSeleccionadas, miCliente,
